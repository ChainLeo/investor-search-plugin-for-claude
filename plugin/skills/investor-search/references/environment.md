...
**All writes go through `scripts/store.py`** (`SKILL.md`, *Claude*); this file describes what
...
**Step 2 is how a user who cannot open the path still gets the file.** In Claude that is the
outputs directory, or a file-sending tool, and it reaches the conversation the run came from;
in a terminal the path is all there is (`SKILL.md`, *Claude*). Check which one this
...
**Writing silently goes somewhere else.** In Claude this is a session whose workspace is thrown away when the session ends, or a terminal and a cloud session with different working directories: the write succeeds and the next run looks somewhere else. The symptom is
...
does not (`SKILL.md`, *Before Job 0* §6). The fix the user can make is on the Claude side: a folder
that persists — the user's own directory, or a folder they connect — for each user or team.
