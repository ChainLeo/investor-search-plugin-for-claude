...
Upload the skill folder in **Customize → Skills → Add**, or drop it into your
skills directory. Do not edit the
...
disk. So it puts them somewhere predictable — in the agent's workspace (by
your own working directory in a terminal, or a folder you connect in Cowork) — and looks there first:
...
**One thing to know about where the files go.** In a terminal they land in your own working
directory and stay there. In Cowork they land in the session workspace, which is **thrown away
when the session ends** — so connect a folder from your computer and the skill writes there
instead. With no folder connected it says so before it starts, and the Excel file becomes the
memory: upload it next time and the run continues from it.
